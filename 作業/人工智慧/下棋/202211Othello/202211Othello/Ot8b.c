...
int  Compute_Grades(int flag, int myturn);
...
//---------------------------------------------------------------------------
void printboard_weight()
{
    for(int i = 0;i<Board_Size;i++)
        {
            for(int j = 0;j<Board_Size;j++)
                printf("%d ",board_weight[i][j]);
            printf("\n");
        }
...
int Compute_Grades(int flag,int myturn)
{
    int i,j;
    int B,W, BW, WW;
    int myMoves = Find_Legal_Moves( Stones[myturn] );
    int youMoves = Find_Legal_Moves( 1-Stones[myturn] );
    B = BW = W = WW = 0;
...
//********************************
    ...
    int g = -1;
//*****************************************change deep
    if(HandNumber >= 0 && HandNumber<8)
    {
        search_deep = 6;
    }else if (HandNumber >= 8 && HandNumber<46)
    {
        search_deep = 8;
    }else
    {
...
//************************************************ Stabilizer
    ...
    if(Now_Board[0][0]==Stones[myturn]&&lu==0)
    {
        board_weight[0][1]=board_weight[1][1]=board_weight[1][0]=25;
        for(int i = 1;i<Board_Size;i++)
        {
            board_weight[0][i]+=Board_Size-i;
            board_weight[i][0]+=Board_Size-i;
        }
        lu=1;
        printboard_weight();
    }
    if(Now_Board[7][7]==Stones[myturn]&&rd==0)
    {
        board_weight[7][6]=board_weight[6][6]=board_weight[6][7]=25;
        for(int i = 1;i<Board_Size;i++)
        {
            board_weight[7][i]+=i;
            board_weight[i][7]+=i;
    ...
    if(Now_Board[7][0]==Stones[myturn]&&ld==0)
    {
        board_weight[7][1]=board_weight[6][1]=board_weight[6][0]=25;
        for(int i = 1;i<Board_Size;i++)
        {
            board_weight[7][i]+=i;
            board_weight[i][0]+=Board_Size-i;
    ...
    if(Now_Board[0][7]==Stones[myturn]&&ru==0)
    {
        board_weight[0][6]=board_weight[1][6]=board_weight[1][7]=25;
        for(int i = 1;i<Board_Size;i++)
        {
            board_weight[7][i]+=i;
            board_weight[i][0]+=Board_Size-i;
